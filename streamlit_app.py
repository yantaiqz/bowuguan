...
<style>
    ...
    .t-title { font-size: 1rem; font-weight: 600; margin-bottom: 6px !important; }
        ...
        def render_pay_tab(title, amount_str, color_class, img_name, qr_suffix, link=None):
            with st.container(border=True):
                ...
                if link:
                    ...
                    st.markdown(f"""<div class="pay-instruction" style="text-align: center;">{current_text['detail_empty'].split('，')[1] if st.session_state.language == 'zh' else 'Scan the QR code above with your phone'}</div>""", unsafe_allow_html=True)
